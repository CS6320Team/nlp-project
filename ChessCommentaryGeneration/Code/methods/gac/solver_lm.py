...
class Solver:
    ...
    def score(self, idList):
        test_tgt_batches, test_tgt_masks = torch_utils.splitBatches(train=[idList, ], batch_size=1,
                                                                    padSymbol=self.cnfg.garbage, method="post")
    ...
    def main(self):
        ...
        if cnfg.mode == "train" or cnfg.mode == "LM":
            ...
            # Also on valid
            valid_src = srcLangObj.read_corpus(mode="valid")
            valid_src_batches, valid_src_masks = torch_utils.splitBatches(train=valid_src, batch_size=1,
                                                                          padSymbol=cnfg.garbage, method="pre")
