...
class Solver:
    ...
    def splitData(self, data, batch_size=-1):
        ...
        all_masks = []
        for j in range(m):
            src, src_masks = torch_utils.splitBatches(train=[data_input[j] for data_input in data_inputs],
                                                      batch_size=batch_size, padSymbol=cnfg.garbage, method="pre")
            all_data.append(src)
            all_masks.append(src_masks)
        tgt_batches, tgt_masks = torch_utils.splitBatches(train=data_targets, batch_size=batch_size,
    ...
    def main(self, typ="two_tuple"):
        ...
        if typ == "two_tuple":
            model = MultiSeqToSeqAttn(cnfg, wids_src=wids_src, wids_tgt=wids_tgt,
                                      encoder_configurations=['birnn', 'birnn'], typ=typ)
        elif typ == "entire_as_tuple":
            model = MultiSeqToSeqAttn(cnfg, wids_src=wids_src, wids_tgt=wids_tgt, encoder_configurations=['birnn'],
                                      typ=typ)
        elif typ == "three_tuple":
            model = CNNSeqToSeqAttn(cnfg, wids_src=wids_src, wids_tgt=wids_tgt,
                                    encoder_configurations=['cnn', 'cnn', 'birnn'], typ=typ)
        ...
        if cnfg.mode == "train" or cnfg.mode == "trial":
                ...
                for epochId in range(cnfg.NUM_EPOCHS):
                    ...
                    for j, batchId in enumerate(batch_indices):
                        ...
                        if j % cnfg.PRINT_STEP == 0:
                            ...
                            print("Epoch:", epochId, " Total Validation Loss:", totalValidationLoss, " Perplexity:",
                                  perplexity)
            ...
            model.decodeAll(test_src_batches, test_src_masks, modelName, method="greedy", evalMethod="BLEU",
                            suffix="test")
            # Also on valid
