...
class MultiSeqToSeqAttn(nn.Module):
    def __init__(self, cnfg, wids_src=None, wids_tgt=None, encoder_configurations=['birnn'],
                 typ="two_tuple"):  # As of now assumes shared vocab on all input sequences
        ...
        for j, encoder_config in enumerate(encoder_configurations):
            ...
            if encoder_config == "birnn":
                encoder = EncoderBiRNNModel(self.wids_src, self.cnfg.srcVocabSize, self.cnfg.emb_size,
                                            self.cnfg.hidden_size, self.cnfg.use_LSTM, True, shared_embeddings)
            elif encoder_config == "rnn":
                encoder = EncoderRNNModel(self.wids_src, self.cnfg.srcVocabSize, self.cnfg.emb_size,
                                          self.cnfg.hidden_size, self.cnfg.use_LSTM, True, shared_embeddings)
            self.configurations[s] = encoder_config
            self.models[s] = encoder
            self.add_module(s, encoder)

        self.decoder = AttnDecoderRNNMutipleInput(self.wids_tgt, cnfg.tgtVocabSize, cnfg.emb_size, cnfg.hidden_size,
                                                  cnfg.use_LSTM, cnfg.use_attention, cnfg.share_embeddings,
                                                  reference_embeddings=shared_embeddings,
                                                  extra_dims=self.cnfg.hidden_size, typ=self.typ)
    ...
    def decodeAll(self, src_batches, src_masks, modelName, method="greedy", evalMethod="BLEU", suffix="test"):
        ...
        for batchId in batch_indices:
            ...
            if method == "greedy":
                tgtString = self.forward(srcBatchList=src_batch, srcMaskList=src_mask,
                                         mode="beam_decode")  # "beam_decode")
    ...
    def _decoderStep(self, batch_size, cur_inputs, encoder_outputs, o_t, previous_hidden, inference=False):
        ...
        out, hidden, c_t = self.models['decoder'](batch_size, tgtEmbedIndex, encoder_outputs, o_t, previous_hidden,
                                                  feedContextVector=False, inference=inference)
        return out, hidden, c_t

    def forward(self, srcBatchList, trgtBatch=None, srcMaskList=None, trgtMask=None, loss_function=None,
                inference=False, mode="train"):  # mode: train, greedy_decode, beam_decode
        ...
        for j in range(len(srcBatchList)):  # each is b,seqlen
            ...
            enc_out, hidden_val = self.models[s](srcBatchList[j].T, srcMaskList[j].T, hidden=hidden,
                                                 inference=inference, srcMasking=self.cnfg.srcMasking)
        ...
        if self.typ == "two_tuple" or self.typ == "three_tuple":  # for 3-tuple, only first two (boards) are used. third (move) is used to init decoder instead
            encoder_outs_combined = self.models['decoder'].combineEncoderOutputs(
                encoder_outs_tensors)  # probably: seq length * b * hidden_size
        ...
        out, hidden, c_0 = self.models['decoder'](batch_size, tgtEmbedIndex, None, None, hidden, feedContextVector=True,
                                                  contextVector=c_0)
        ...
        if mode == "train":
            ...
            for _ in range(2 * srcSentenceLength + 10):  # self.cnfg.TGT_LEN_LIMIT:
                ...
                out, hidden, context_vectors = self._decoderStep(row_beam.shape[0], row_beam,
                                                                 encoder_outs_combined_beam, o_t_beam, hidden_beam,
                                                                 inference=True)
                out = out.squeeze(0)
                if self.cnfg.use_attention:
