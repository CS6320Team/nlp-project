...
class CNNSeqToSeqAttn(nn.Module):
    def __init__(self, cnfg, wids_src=None, wids_tgt=None, encoder_configurations=['cnn', 'cnn', 'birnn'],
                 typ="normal"):  # As of now assumes shared vocab on all input sequences
        ...
        s = "encoder_prevboard"
        self.encoder_prevboard = CNN(inp_channels=src_emb_size, output_channels=output_channels,
                                     kernel_size=kernel_size)
        s = "encoder_curboard"
        self.encoder_curboard = CNN(inp_channels=src_emb_size, output_channels=output_channels, kernel_size=kernel_size)
        s = "encoder_move"
        self.encoder_move = EncoderRNNModel(self.wids_src, self.cnfg.srcVocabSize, src_emb_size, self.src_hidden_size,
                                            self.cnfg.use_LSTM, True, self.encoder_embeddings)

        # Decoder
        self.decoder = AttnDecoderRNNMutipleInputCNN(self.wids_tgt, cnfg.tgtVocabSize, cnfg.emb_size, cnfg.hidden_size,
                                                     cnfg.use_LSTM, cnfg.use_attention, cnfg.share_embeddings,
                                                     reference_embeddings=None, extra_dims=self.src_hidden_size,
                                                     cnn_output_size=cnn_output_size, typ=self.typ)
    ...
    def _decoderStep(self, batch_size, cur_inputs, encoder_outputs, o_t, previous_hidden, inference=False):
        ...
        out, hidden, c_t = self.decoder(batch_size, tgtEmbedIndex, encoder_outputs, o_t, previous_hidden,
    ...
    def forward(self, srcBatchList, trgtBatch=None, srcMaskList=None, trgtMask=None, loss_function=None,
        ...
        enc_out, hidden_val = self.encoder_move(move.T, srcMaskList[2].T, hidden=hidden, inference=inference,
                                                srcMasking=self.cnfg.srcMasking)
        ...
        c_0, hidden = self._initDecoder(hidden_vals[-1], (batch_size, self.src_hidden_size + 2 * self.cnn_output_size),
                                        True, (1, batch_size, self.hidden_size))  # init with move
        ...
        out, hidden, c_0 = self.decoder(batch_size, tgtEmbedIndex, None, None, hidden, feedContextVector=True,
                                        contextVector=c_0)
