...
class CLM(nn.Module):
    ...
    def __init__(self, params, start_symbol, end_symbol):
        ...
        self.decoder = AttnDecoderRNN(vocab_size=params.vocab_size, emb_size=params.decoder_emb_size,
                                      hidden_size=params.decoder_hidden_size,
                                      encoder_feat_size=params.encoder_feat_size)
    ...
    def _beamDecode(self, encoder_outputs, max_len_decode, previous_output, previous_hidden):
        ...
        for _ in range(max_len_decode):  # self.cnfg.TGT_LEN_LIMIT:
            ...
            row_beam = self._getLongVariable(
                row_beam)  # np.array(row_beam) #self.getIndex(row, inference=True) #np.array(row)
    ...
    def _greedyDecode(self, encoder_outputs, max_len_decode, previous_output, previous_hidden):
        ...
        for t in range(max_len):
            # print "current_input = ", current_input
            # current_input = self._getLongVariable(current_input)
            previous_output, previous_hidden = self._decoderStep(current_input, encoder_outputs, previous_output,
                                                                 previous_hidden)
    ...
    def forward(self, encoder_feats, gt_output=None, mode="train", loss_function=None, \
        ...
        if mode == "train":
            ...
            cross_entropy_loss = sum(
                [loss_function(output, tgt) for output, tgt in zip(outputs, gt_outputs_list_tensor)])
            ...
            if decoding_type == "greedy":
                ...
                # gt_output: B,len
                outputs, output_dist = self._greedyDecode(encoder_outputs, max_len_decode, previous_output,
                                                          previous_hidden)
                if get_loss:
                    gt_outputs_list_tensor = [self._getLongVariable(val) for val in gt_output]
                    cross_entropy_loss = sum([loss_function(output, tgt) for output, tgt in zip(output_dist,
                                                                                                gt_outputs_list_tensor)])  # what if target length is greater than output #TODO
                    loss = cross_entropy_loss
                    return loss, outputs
                else:
                    return outputs
    ...
    def _decoderStep(self, current_input, encoder_outputs, previous_output, previous_hidden):
        ...
        out, hidden, context_vector, attn_weights = self.decoder(batch_size, current_input, encoder_outputs,
                                                                 previous_output, previous_hidden)
        return out, hidden
