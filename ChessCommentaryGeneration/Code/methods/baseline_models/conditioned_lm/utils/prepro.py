...
class Prepro:
    ...
    def _getFeats(self, split):
        ...
        for i, mask in enumerate(feats_mask):
            if mask == 1:
                ...
                ret.append(feats)
                logging.debug(
                    "[_getFeats(): split:" + split + "; feat type: " + feat_types[i] + " feats: " + str(feats.shape))
        ret = np.hstack(ret)
        return ret
    ...
    def getData(self, src_dir="../../../data/", typ="che-eng.0", lang="en"):
        ...
        logging.info(
            "[prepro.getData()]: train data sample = " + str(self._getSentenceRepresentation(self.train_split[23][0])))
