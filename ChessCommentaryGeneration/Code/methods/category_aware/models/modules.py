...
class AttnDecoderRNN(nn.Module):
    def __init__(self, wids, vocabSize, emb_size, hidden_size, use_LSTM, use_attention, share_embeddings,
                 reference_embeddings=None, sigmoid=False, category_feature_size=1):
    ...
    def forward(self, batchSize, tgtEmbedIndex, encoderOutTensor, o_t, hidden, feedContextVector=False,
                contextVector=None, inference=False, getAtt=False, categories=None):
        ...
        if self.use_attention:
            if not feedContextVector:
                ...
                if not getAtt:
                    if not self.sigmoid:
                        alphas = torch.transpose(F.softmax(dotProduct), 0, 1).unsqueeze(2).expand(
                            encoderOutTensor.size())
                    else:
                        alphas = torch.transpose(F.sigmoid(dotProduct), 0, 1).unsqueeze(2).expand(
                            encoderOutTensor.size())
                else:
                    if not self.sigmoid:
