...
def expander_main():
    ...
    pickle.dump(counts, open("extra_pages.p", "wb"))
    ...
    def _getBoardValues(self, soup):
        ...
        for div in divs:
            ...
            for ele in board_elements:
                ...
                ele_style_vals = {val.split(": ")[0].strip(): val.split(": ")[1].strip().replace("px", "") for val in
                                  ele_style_vals if val.count(": ") > 0}
                if 'left' in ele_style_vals:
                    ...
                    if len(ele_img) > 0:
                        ...
                        ele_style_vals = {val.split(": ")[0].strip(): val.split(": ")[1].strip().replace("px", "") for
                                          val in ele_style_vals if val.count(": ") > 0}
                        if 'left' in ele_style_vals:
    ...
    def getData(self):
        ...
        for file in all_files:
            try:
                ...
                results2 = [result for result in results2 if len(result.findAll("td", recursive=False)) == 2 and len(
                    self._utils.getDivOfClass(result, "cdiag_frame")) > 0]  # based on observation
                all_steps_info = []
