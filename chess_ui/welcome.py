...
import random
import threading
from PyQt6.QtWidgets import (QApplication, QVBoxLayout, QWidget, QLabel, QPushButton, QHBoxLayout, QTextEdit, QScrollArea, QFrame, QListWidget, QStackedLayout)
...
class WelcomeScreen(QWidget):
    ...
    def start_app(self):
        ...
        if self.mode_dropdown.currentText() == "analyze a game":
            mode = 'analysis'
        elif self.mode_dropdown.currentText() == "train puzzles":
            mode = "puzzles"
        else:
            mode = "bot"
...
class MainApp(QWidget):
            ...
            if self.awaiting_best_move:
                ...
                if user_move.lower() == "answer":
                    ...
                    if user_move == self.curr_best_move:
                        ...
                        dummy_board.pop()
                        #pass this state of dummy board as prev state
                        dummy_board.push(chess.Move.from_uci(user_move))
                        threading.Thread(target=self.analysis_walkthrough).start()
                        #pass state as post state
                    else:
    ...
    def prepare_analysis(self):
        ...
        for san in self.move_history:
            move = local_board.parse_san(san)
            uci_move = move.uci()
    ...
    def analysis_walkthrough(self):
        ...
        for move_idx, uci_move in enumerate(self.uci_notation[len(self.curr_moves):]):
            ...
            if curr_best_move_eval[0] != player_move_eval[0] or abs(curr_best_move_eval[1] - player_move_eval[1]) > 15: #play w this
                ...
                self.add_to_chat("Coach", f"Here you made a blunder by moving {uci_move}, can you find the best move?") #need to move the piece back and update the board (remove the move from move list)
    ...
    def initUI(self):
        ...
        self.move_input.setPlaceholderText("Enter your move (e.g., e2e4, b1c3)...")
...
class App(QWidget):
    def __init__(self, puzzles_df=None):
        super().__init__()
        self.puzzles_df = pd.read_csv("C:\\Users\\wei0c\\Desktop\\school\\7-1\\CS-6320-NLP\\lichess_db_puzzle.csv")
        self.initUI()

    def initUI(self):
        self.setWindowTitle("Chess Application")
        self.setMinimumSize(1300, 800) # Make the application wider
